...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     //passTrigger_ = HLT_MuPT_eta2p1_passTrigger();
     ...
     //  -- LEPTON SKIM - Exactly 1 loose muon, 2+ jets over 100GeV
     ...
     for(unsigned int iJetR=0; iJetR<PFJetPt->size(); iJetR++){
       if( jetRLooseCheck(iJetR) && PFJetPt->at(iJetR)>70 ) CountLooseJets+=1;
     }
     if( CountLooseLeptons==1 && CountLooseJets>1 ) isOfflineLepton_=1;
