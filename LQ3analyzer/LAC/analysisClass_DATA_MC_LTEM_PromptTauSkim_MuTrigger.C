...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     //  -- LEPTON SKIM - Exactly 1 LI Muon and at least 1 LI Tau, no charge requirement
     ...
     for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
       ...
       Mu.SetPtEtaPhiM( MuonPt->at(iMuR1),  MuonEta->at(iMuR1),  MuonPhi->at(iMuR), 0 );
       CountLooseLeptons+=1;
       for(unsigned int iTauR=0; iTauR<HPSTauPt->size(); iTauR++){
	 if( !tauRLooseCheck(iTauR) ) continue;
	 Tau1.SetPtEtaPhiM( HPSTauPt->at(iTauR),  HPSTauEta->at(iTauR),  HPSTauPhi->at(iTauR), 0 );
	 if( fabs((Mu1+Mu2).M()-Zmass)<20 ) isZToMuMu_=true;
       }
     }
