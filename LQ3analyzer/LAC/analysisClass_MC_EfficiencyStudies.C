...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     for( unsigned int iMuT=0; iMuT<GenZMuPt->size(); iMuT++){
       if( GenZMuPt->at(iMuT)<25         ) continue;
       if( fabs(GenZMuEta->at(iMuT))>2.1 ) continue;
       NGenMuon++;
       // Is Matched to a Reco Muon?
       GenMuon.SetPtEtaPhiM( GenZMuPt->at(iMuT),     GenZMuEta->at(iMuT),   GenZMuPhi->at(iMuT), 0 );
     ...
     for( unsigned int iTauT=0; iTauT<GenZTauPt->size(); iTauT++){
       if( GenZTauTauVisiblePt->at(iTauT)<20          ) continue;
       if( fabs(GenZTauTauVisibleEta->at(iTauT))>2.1  ) continue;
       if( GenZTauTauDecayMode->at(iTauT)<3 ) continue;//mode 1: electron, 2: muon (semileptonic)
       NGenTau++;
       // Is Matched to a Reco Tau?
       GenTau.SetPtEtaPhiM( GenZTauPt->at(iTauT), GenZTauEta->at(iTauT), GenZTauPhi->at(iTauT), 0 );
     ...
     for( unsigned int iMuT=0; iMuT<GenZMuPt->size(); iMuT++){
       if( GenZMuPt->at(iMuT)<25         ) continue;
       if( fabs(GenZMuEta->at(iMuT))>2.1 ) continue;
       for( unsigned int iMuT2=(iMuT+1); iMuT2<GenZMuPt->size(); iMuT2++){
	 if( GenZMuPt->at(iMuT2)<25         ) continue;
	 if( fabs(GenZMuEta->at(iMuT2))>2.1 ) continue;
	 GenMuon.SetPtEtaPhiM( GenZMuPt->at(iMuT),     GenZMuEta->at(iMuT),   GenZMuPhi->at(iMuT), 0 );
	 GenMuon2.SetPtEtaPhiM( GenZMuPt->at(iMuT2),     GenZMuEta->at(iMuT2),   GenZMuPhi->at(iMuT2), 0 );
     ...
     for( unsigned int iTauT=0; iTauT<GenZTauPt->size(); iTauT++){
       ...
       if( fabs(GenZTauTauVisibleEta->at(iTauT))>2.1  ) continue;
       if( GenZTauTauDecayMode->at(iTauT)<3 ) continue;//mode 1: electron, 2: muon (semileptonic)
       for( unsigned int iTauT2=(iTauT+1); iTauT2<GenZTauPt->size(); iTauT2++){
         if( GenZTauTauVisiblePt->at(iTauT2)<20          ) continue;
         if( fabs(GenZTauTauVisibleEta->at(iTauT2))>2.1  ) continue;
         if( GenZTauTauDecayMode->at(iTauT2)<3           ) continue;//mode 1: electron, 2: muon (semileptonic)
         GenTau.SetPtEtaPhiM( GenZTauPt->at(iTauT), GenZTauEta->at(iTauT), GenZTauPhi->at(iTauT), 0);
	 GenTau2.SetPtEtaPhiM( GenZTauPt->at(iTauT2), GenZTauEta->at(iTauT2), GenZTauPhi->at(iTauT2), 0);
