   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
       ...
       for(unsigned int iJetR=0; iJetR<PFJetPt->size(); iJetR++){
         ...
         if( mucand.DeltaR(jetcand)<muJetDeltaRmin ){ muJetDeltaRmin=mucand.DeltaR(jetcand); iMuJetR=iJetR; }//picks the smallest deltaR and JET
	 //if( mucand.DeltaR(jetcand)<0.3 ) cout<< "Mu - Jet: "<<mucand.DeltaR(jetcand)<<endl;
       }
       //cout<< "Mu - Jet: "<<muJetDeltaRmin<<"     JetEnergy/MuEnergy="<<setprecision(2)<<PFJetEnergy->at(iMuJetR)/MuonEnergy->at(iMuR)<<endl;
       if( muJetDeltaRmin>=0.2 ) iMuJetR=999;//checks if nearest JET is near enough!
       //if( muJetDeltaRmin>=0.5 ) iMuJetR=999;//checks if nearest JET is near enough!
	 ...
	 //if( (PFJetMuonEnergyFraction->at(iJetR)>0.9 && mucand.DeltaR(jetcand)<0.5) || mucand.DeltaR(jetcand)<0.15 )  continue;
	 //if( PFJetMuonEnergyFraction->at(iJetR)>0.84 && mucand.DeltaR(jetcand)<0.5 )  continue;
	 //if( PFJetMuonEnergyFraction->at(iJetR)>0.84 && mucand.DeltaR(jetcand)<0.5 )  continue;
	 ...
	 //
	 /*if(  mucand.DeltaR(jetcand)<0.1 ){
	   int pass=0;
	   if(  (( MuonPFIsoR04ChargedHadron->at(iMuR) +
		   TMath::Max( 0.0 , (MuonPFIsoR04NeutralHadron->at(iMuR)+MuonPFIsoR04Photon->at(iMuR)-0.5*MuonPFIsoR04PU->at(iMuR)) ))
		 /MuonPt->at(iMuR))<0.12 ) pass=1;
	   cout<<" JetMuEnergyFrac: "<<PFJetMuonEnergyFraction->at(iJetR)<<"  P/F: "<<pass<<"  Ncons:"<<PFJetNConstituents->at(iJetR)<<endl;
	   }*/
	 //
         if( mucand.DeltaR(jetcand)<muJetDeltaRmin ){ muJetDeltaRmin=mucand.DeltaR(jetcand);}
       }
       probeMuJetDeltaRminArray.push_back(muJetDeltaRmin);
       //
       //
       //if( fabs(muJetDeltaRmin-1.3)<0.2 && JetCounter()>1 ){
       //cout<<" MuN: "<<MuCounter()<<endl;
       //cout<<"TauN: "<<TauCounter()<<endl;
       //cout<<" ElN: "<<ElCounter()<<endl;
       //cout<<"JetN: "<<JetCounter()<<endl;
       ////cout<<"Muon= "<< MuonPt->at(iMuR)<<" "<<MuonEta->at(iMuR)<<" "<<MuonPhi->at(iMuR)<<" NearJetMuEnergyFrac: "<<PFJetMuonEnergyFraction->at(iMuJetR)<<endl;
       //cout<<"Muon= "<< MuonPt->at(iMuR)<<" "<<MuonEta->at(iMuR)<<" "<<MuonPhi->at(iMuR)<<endl;
       //// GOTTA COUNT JETS WITH MU FRAC <0.7 MATCHING WITH THE MUON.
       //TLorentzVector muv, jetv;
       //for(unsigned int ijet=0; ijet<PFJetPt->size(); ijet++){
       //if( !jetRisoCheck(ijet) ) continue;
       //muv.SetPtEtaPhiM(MuonPt->at(iMuR),MuonEta->at(iMuR),MuonPhi->at(iMuR),0);
       //jetv.SetPtEtaPhiM( PFJetPt->at(ijet),  PFJetEta->at(ijet),  PFJetPhi->at(ijet),  0 );
       //cout<<" Jet="<<PFJetPt->at(ijet)<<" "<<PFJetEta->at(ijet)<<" "<<PFJetPhi->at(ijet)<<"   "<<muv.DeltaR(jetv)<<"/"<<muv.DeltaPhi(jetv)<<endl;
       //}
       //}
       //
       //
	 ...
	 if( taucand.DeltaR(jetcand)<tauJetDeltaRmin ){ tauJetDeltaRmin=taucand.DeltaR(jetcand); iTauJetR=iJetR; }//picks the smallest deltaR and JET
       ...
       //cout<< "Tau - Jet: "<<tauJetDeltaRmin<<"   JetPt/TauPt="<<PFJetPt->at(iTauJetR)/HPSTauPt->at(iTauR)<<endl;
	 ...
	 if( fabs(PFJetEta->at(iJetR))>3 ) continue;
       ...
       probeTauJetDeltaRminArray.push_back(tauJetDeltaRmin);
       //
     }
     ...
     //for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
     //if( !muRisoCheck(iMuR)  ) continue;
     //if( MuonPt->at(iMuR)<25 ) continue;
     //if( MuonPt->at(iMuR)>40 ) continue;
     //cout<<endl;
     //cout<<" IsoMuon"<<endl;
     //for(unsigned int iJetR1=0; iJetR1<PFJetPt->size(); iJetR1++){
     //if( PFJetPt->at(iJetR1)<20       ) continue;
     //if( fabs(PFJetEta->at(iJetR1))>3 ) continue;
     //TLorentzVector jetvec, muvec;
     //muvec.SetPtEtaPhiM( MuonPt->at(iMuR),  MuonEta->at(iMuR),  MuonPhi->at(iMuR),  0 );
     //jetvec.SetPtEtaPhiM( PFJetPt->at(iJetR1),  PFJetEta->at(iJetR1),  PFJetPhi->at(iJetR1),  0 );
     //if( muvec.DeltaR(jetvec)<0.55 )cout<<" mu-jet DeltaR: "<<muvec.DeltaR(jetvec)<<"     MuFrac:"<<PFJetMuonEnergyFraction->at(iJetR1)<<"  ("<<
     //MuonEnergy->at(iMuR)/PFJetEnergy->at(iJetR1)<<")"<<"  --  "<<
     //fabs(1-((MuonEnergy->at(iMuR)/PFJetEnergy->at(iJetR1))/PFJetMuonEnergyFraction->at(iJetR1)))<<endl;
     //}
     //}
	 ...
	 //
	 // For Signal MC, these two collections overlap 100% (as expected)
	 //for( unsigned int iTauT=0; iTauT<GenLQTopTauPt->size(); iTauT++){
	 //cout<<" LQ->Top->W->Tau: "<<GenLQTopTauPt->at(iTauT)<<" / "<<GenLQTopTauEta->at(iTauT)<<endl;
	 //}
	 //for( unsigned int iTauT=0; iTauT<GenWTauPt->size(); iTauT++){
	 //cout<<"          W->Tau: "<<GenWTauPt->at(iTauT)<<" / "<<GenWTauEta->at(iTauT)<<endl;
	 //}
	 for( unsigned int iTauT=0; iTauT<GenLQTauTauPt->size(); iTauT++){
	   ...
	   if( GenLQTauTauPdgId->at(iTauT)>0 ) charge=-1;
	   if( GenLQTauTauPdgId->at(iTauT)<0 ) charge=+1;
	 ...
	 for( unsigned int iMuT=0; iMuT<GenLQTauMuonPt->size(); iMuT++){
	   mugen.SetPtEtaPhiM( GenLQTauMuonPt->at(iMuT),     GenLQTauMuonEta->at(iMuT),   GenLQTauMuonPhi->at(iMuT), 0 );
	   if( GenLQTauMuonPdgId->at(iMuT)>0 ) charge=-1;
	   if( GenLQTauMuonPdgId->at(iMuT)<0 ) charge=+1;
	 ...
	 for( unsigned int iElT=0; iElT<GenLQTauElectronPt->size(); iElT++){
	   elgen.SetPtEtaPhiM( GenLQTauElectronPt->at(iElT),     GenLQTauElectronEta->at(iElT),   GenLQTauElectronPhi->at(iElT), 0 );
	   if( GenLQTauElectronPdgId->at(iElT)>0 ) charge=-1;
	   if( GenLQTauElectronPdgId->at(iElT)<0 ) charge=+1;
