...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // -- RUN LTEM 
     which_MuMu(ltemMuTau);
     //which_MuTau(ltemMuTau);
     //which_MuTauOS(ltemMuTau);
     ...
     // ---- trigger type
     // need to decide if this is a Mu40 event or a Mu24 event (disabled for now)
     ...
     //TriggerEfficiencyWeights_=IsoMu24e2p1_Eff( MuonPt->(triggerMuon), MuonEta->at(triggerMuon) );
     ...
     //if( ST()>500 && JetCounter()>1 && ltemMuTau.size()==2 &&
     if( //ST()>500 && JetCounter()>1 && ltemMuTau.size()==2 &&
	 ST()>500 && JetCounter()>1 && ltemMuTau.size()==2 &&
	 muJetDeltaRmin(ltemMuTau[0])>0.5 && muJetDeltaRmin(ltemMuTau[1])>0.5 ) OfflineCuts_=1; // <<<< CONTROL REGION with no DATA
	 ...
	 passedCut("PassAllMuHLTmatching") && passedCut("PassOfflineCuts") ){ 
       ...
       //cout<<"("<<jentry<<")  muPR: "<<muPR(ltemMuTau[0])<< "    muPRE: "<< muPRE(ltemMuTau[0])<<endl;
       //cout<<"("<<jentry<<")  muFR: "<<muFR(ltemMuTau[0])<< "    muFRE: "<< muFRE(ltemMuTau[0])<<endl;
       //cout<<"("<<jentry<<") tauPR: "<<tauPR(ltemMuTau[1])<<"   tauPRE: "<< tauPRE(ltemMuTau[1])<<endl;
       //cout<<"("<<jentry<<") tauFR: "<<tauFR(ltemMuTau[1])<<"   tauFRE: "<< tauFRE(ltemMuTau[1])<<endl;
       ...
       if( muRTightCheck(ltemMuTau[1]) ) isTauTight_=true;
       int LTEMtype=0;
       //..........................................//MT
	 ...
	 if(  isRecoMuPrompt_ &&  isRecoTauPrompt_ ) TTGenhisto->Fill( 4, w );//PP
       ...
       LTEMMuJetDeltaRminhisto->Fill( muJetDeltaRmin(ltemMuTau[0]) );
       LTEMTauJetDeltaRminhisto->Fill( tauJetDeltaRmin(ltemMuTau[1]) );
       LTEMMuPthisto->Fill( MuonPt->at(ltemMuTau[0]) );
       LTEMTauPthisto->Fill( HPSTauPt->at(ltemMuTau[1]) );
       LTEMMuEtahisto->Fill( MuonEta->at(ltemMuTau[0]) );
       LTEMTauEtahisto->Fill( HPSTauEta->at(ltemMuTau[1]) );
       LTEMMuPhihisto->Fill( MuonPhi->at(ltemMuTau[0]) );
       LTEMTauPhihisto->Fill( HPSTauPhi->at(ltemMuTau[1]) );
       ...
       LTEMMuTauDeltaRhisto->Fill( LTEMMuVector.DeltaR(LTEMTauVector) );
       LTEMMuTauDeltaPhihisto->Fill( LTEMMuVector.DeltaPhi(LTEMTauVector) );
       LTEMMuTauMasshisto->Fill( (LTEMMuVector+LTEMTauVector).M() );
