...
bool analysisClass::tauRLooseCheck(unsigned int iTauR){
  ...
  if( tauJetDeltaRmin(iTauR)<0.7 ) return false;//jul-31-2013 (loose-tight)
  //
  return true;
}
// // //
...
bool analysisClass::muRLooseCheck(unsigned int iMuR){
  ...
  if( muJetDeltaRmin(iMuR)<0.5 ) return false;//jul-31-2013 (loose-tight)
  //
  return true;
}
// // //
...
double analysisClass::tauFR( unsigned int iTauR ){
  ...
  // lambda=0.5 +\- 0.5
  if( tauJetDeltaRmin_>=1.0 && !isBarrel_ ) return 0.164*tauFRdR3E_SF;//0.163*tauFRdR3E_SF;
  if( tauJetDeltaRmin_>=0.8 && !isBarrel_ ) return 0.151*tauFRdR2E_SF;//0.147*tauFRdR2E_SF;
  if( tauJetDeltaRmin_>=0.7 && !isBarrel_ ) return 0.133*tauFRdR1E_SF;//0.123*tauFRdR1E_SF;
  //
  return 0;
}
// // //
...
double analysisClass::tauFRE( unsigned int iTauR ){
  ...
  //lambda=0.5 +\- 0.5
  if( tauJetDeltaRmin_>=1.0 && !isBarrel_ ) return QuadSum( 0.003/0.164, tauFRdR3E_SFerr/tauFRdR3E_SF )*tauFR(iTauR);
  if( tauJetDeltaRmin_>=0.8 && !isBarrel_ ) return QuadSum( 0.011/0.151, tauFRdR2E_SFerr/tauFRdR2E_SF )*tauFR(iTauR);
