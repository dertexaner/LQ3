...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     double LooseJetHT=0;
     //double QuadJetSTave=0;
     double leadingJetPt=0;
     for(unsigned int iJetR=0; iJetR<PFJetPt->size(); iJetR++){
       if( jetRLooseCheck(iJetR) ){
	 leadingJetPt=PFJetPt->at(iJetR);
	 break;
       }
     }
     for(unsigned int iJetR=0; iJetR<PFJetPt->size(); iJetR++){
     ...
     if( CountLooseLeptons>0 ) isOfflineLepton_=1;
	 ...
	 passedCut("PassHLTmatching") && passedCut("PassOfflineCuts") ){ 
       ...
       double MuonPfIso;
       for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
	 if( MuonPt ->at(iMuR)<35 ) continue;
	 if( !muRisoCheck(iMuR)   ) continue;
	 probeMu=iMuR;
	 LooseMuonPthisto->Fill(  MuonPt ->at(iMuR), w );
	 LooseMuonEtahisto->Fill( MuonEta->at(iMuR), w );
	 LooseMuonPhihisto->Fill( MuonPhi->at(iMuR), w );
	 if( muRTightCheck(iMuR) ){
	   TightMuonPthisto->Fill(  MuonPt ->at(iMuR), w );
	   TightMuonEtahisto->Fill( MuonEta->at(iMuR), w );
	   TightMuonPhihisto->Fill( MuonPhi->at(iMuR), w );
	   MuonPfIso = MuonPFIsoR04ChargedHadron->at(iMuR) +
	   ...
	   if( (0.12*100)>MuonPfIso ){
	     TightMuonPtAbsIsohisto->Fill(  MuonPt ->at(iMuR), w );
	     TightMuonEtaAbsIsohisto->Fill( MuonEta->at(iMuR), w );
	     TightMuonPhiAbsIsohisto->Fill( MuonPhi->at(iMuR), w );
	   }
	 }
	 break;
       }
	 ...
	 if( tauRTightCheck(iTauR) ) eventTauN++;
       }
