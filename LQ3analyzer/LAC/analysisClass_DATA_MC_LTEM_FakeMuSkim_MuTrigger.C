...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     /*
     //  -- LEPTON SKIM - Exactly Three Loose Muons & ZtoMuMu
     int isOfflineLepton_=0;
     int CountLooseLeptons=0;
     for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
       if( MuonPt->at(iMuR)<=MuonPtCut                 ) continue;
       if( fabs(MuonEta->at(iMuR))>MuonEtaCut          ) continue;
       if( MuonIsPF->at(iMuR)==1 && (MuonIsGlobal->at(iMuR)==1 || MuonIsTracker->at(iMuR)==1) ) CountLooseLeptons+=1;
     }
     if( CountLooseLeptons == 3 && isZToMuMu() ) isOfflineLepton_=1;
     fillVariableWithValue("PassOfflineLepton", isOfflineLepton_ );// Returns 0, 1
     */

     //  -- LEPTON SKIM - At Least Three LI Muons & ZtoMuMu
     int isOfflineLepton_=0;
     int CountLooseLeptons=0;
     for(unsigned int iMuR=0; iMuR<MuonPt->size(); iMuR++){
